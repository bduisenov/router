...
public class SplitRouteBuilder<T, P> implements RouterBuilder<T, P> {

    private final DefaultRouteBuilder<T, P> parentRouteBuilder;

    private final DefaultRouteBuilder<T, P> splitRouteBuilder;
    ...
    DefaultRouteBuilder<T, P> addSplitRoute() {
        val _route = parentRouteBuilder.route.flatMap(either -> state(context -> either.map(x -> Tuple(x, List.ofAll(splitter.apply(x))))
